...
- **Rationale:** Under recursion every member must be sniffed by content to decide whether it is a nested archive (containers are always traversed, so a leaf filter cannot pre-empt the sniff). The 7z sniff *materialises* the whole member (py7zr has no streaming API), so the cap has to fire first or the materialisation it guards against has already happened — the cap is a hard safety limit, and "it fired on a member you'd have filtered" is the safe direction to err. Streaming backends (ZIP/tar) peek cheaply, so they filter before the cap with no materialisation; the asymmetry is intrinsic to streaming vs whole-member backends. (An earlier *extension-based* sniff that avoided this materialisation was tried and reverted: deciding nesting by name rather than content let archive-named members bypass the leaf filter, which kept producing edge-case defects across several review panels — the simpler, content-correct peek is worth the documented cap ordering.)
